...
ShaderProgram::ShaderProgram(const char* vertexShaderFile,const char* geometryShaderFile,const char* fragmentShaderFile) {
	...
	glAttachShader(shaderProgramNew,fragmentShader);
	if (geometryShaderFile!=NULL) glAttachShader(shaderProgramNew,geometryShader);
	...
	glGetProgramiv(shaderProgramNew, GL_INFO_LOG_LENGTH,&infologLength);

	if (infologLength > 1)
	{
		infoLog = new char[infologLength];
		glGetProgramInfoLog(shaderProgramNew, infologLength, &charsWritten, infoLog);
		printf("%s\n",infoLog);
		delete []infoLog;
	}
...
ShaderProgram::~ShaderProgram() {
	...
	glDetachShader(shaderProgramNew, vertexShader);
	if (geometryShader!=0) glDetachShader(shaderProgramNew, geometryShader);
	glDetachShader(shaderProgramNew, fragmentShader);
